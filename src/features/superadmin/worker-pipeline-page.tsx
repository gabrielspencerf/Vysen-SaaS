    ...
    tone === "success"
      ...
      : tone === "warning"
        ...
        : tone === "danger"
          ...
          : "text-brand-text";
  return (
    <div className="rounded-lg border border-brand-border/70 bg-brand-surface px-3 py-2 text-center">
...
export async function SuperadminWorkerPipelinePage() {
  const snapshot = await getWorkerPipelineSnapshot();

  const workerOk = snapshot.workerStatus === "ok";
  const workerLabel =
    snapshot.workerStatus === "ok"
      ? "Worker ativo"
      : snapshot.workerStatus === "stale"
        ? "Heartbeat atrasado"
        : "Worker ausente";
    ...
    <PageSection variant="plain" className="px-1 py-0 sm:px-2 md:px-2 md:pt-0 md:pb-0">
      <div className="mx-auto w-full max-w-7xl">
        <DashboardPageHeader
          ...
          description="Arquitetura assincrona: Postgres para persistencia, Redis para filas e processo worker que consome os jobs. Em desenvolvimento: npm run worker:dev. Metricas instantaneas."
          icon={Cable}
          badges={[
        ...
        />
        <nav className="mb-8 mt-6 rounded-2xl border border-brand-border bg-brand-surface/35 p-4" aria-label="Atalhos da pagina">
        ...
        <section id="mapa-relacional" className="mb-12 scroll-mt-24" aria-labelledby="relational-map-heading">
          ...
          <p className="mt-1 max-w-3xl text-sm text-brand-muted">
            Dominios principais do schema e convergencia para o nucleo (worker + Redis + Postgres). Listas
        ...
        <div id="metricas-rapidas" className="scroll-mt-24 border-t border-brand-border pt-10">
          <p className="text-xs font-semibold uppercase tracking-wider text-brand-muted">Saude em tempo real</p>
          <h2 className="mt-1 text-lg font-semibold text-brand-text">Metricas rapidas</h2>
          <p className="mt-1 max-w-3xl text-sm text-brand-muted">
            Mesmo padrao visual da Observabilidade: leitura centralizada por cartao.
          </p>
          <div className="mt-6 grid items-stretch gap-4 sm:grid-cols-2 xl:grid-cols-4">
                  ...
                  {snapshot.lastHeartbeatAt ? (
                    <>
                      ...
                      {snapshot.heartbeatAgeMs !== null
                        ? ` · ha ${Math.round(snapshot.heartbeatAgeMs / 1000)}s`
                        : null}
                    </>
            ...
            <Card className="h-full border-brand-border bg-brand-surface">
              <CardContent className="flex h-full flex-col items-center p-5 text-center">
                ...
                >
                  {snapshot.redisOk ? "Conectado" : "Indisponivel"}
                </p>
            ...
            <Card className="h-full border-brand-border bg-brand-surface">
              <CardContent className="flex h-full flex-col items-center p-5 text-center">
                ...
                <p className="mt-1 text-xs text-brand-muted">Jobs apos tentativas esgotadas — ver logs.</p>
              </CardContent>
            </Card>
          </div>
          ...
          {snapshot.metrics ? (
            <div className="mt-6 rounded-xl border border-brand-border bg-brand-surface/60 p-4">
              <div className="flex items-baseline justify-between">
                <p className="text-xs font-semibold uppercase tracking-wider text-brand-muted">
                  Contadores acumulados (worker)
                </p>
                <p className="text-[11px] text-brand-muted">
                  Reset manual via redis-cli ou ao limpar keys{" "}
                  <span className="font-mono">worker:metrics:*</span>
                </p>
              </div>
              <div className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-5">
                <MetricsCounter label="Processados" value={snapshot.metrics.totals.processed} tone="success" />
                <MetricsCounter label="Falhas" value={snapshot.metrics.totals.failed} tone="warning" />
                <MetricsCounter label="Retries" value={snapshot.metrics.totals.retried} tone="neutral" />
                <MetricsCounter label="DLQ" value={snapshot.metrics.totals.sent_to_dlq} tone="danger" />
                <MetricsCounter
                  label="Reaper"
                  value={snapshot.metrics.totals.reaper_revived}
                  tone="neutral"
                />
              </div>
              <p className="mt-3 text-[11px] text-brand-muted">
                Exporte no formato Prometheus via{" "}
                <span className="font-mono">/api/metrics</span> com header{" "}
                <span className="font-mono">Authorization: Bearer &lt;HEALTH_DETAILS_TOKEN&gt;</span>.
              </p>
            </div>
          ) : null}
        </div>
